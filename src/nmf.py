...
- Displays and saves the top terms per topic
...
TOPIC_RANGE  = range(5, 26, 5)  # Tests 5, 10, 15, 20, 25 topics
N_TOP_WORDS  = 10
RANDOM_STATE = 42

# -------------------------------
# Helper Functions
# (defined at module level so Windows multiprocessing can import them)
# -------------------------------
...
if __name__ == "__main__":
    ...
    # ----------------------------
    print(f"\nSearching over topic range: {list(TOPIC_RANGE)}")
    ...
    for n_topics in TOPIC_RANGE:
        ...
        nmf = NMF(
            ...
            init="nndsvda",   # Better initialisation than default for sparse matrices
            max_iter=300,
        )
    ...
    plt.plot(ns, scores, marker="o", linewidth=2, color="darkorange")
    plt.axvline(x=optimal_n, color="red", linestyle="--", label=f"Optimal n={optimal_n}")
    plt.title("NMF Coherence Score (c_v) by Number of Topics")
    ...
    with open(topics_path, "w") as f:
        f.write("NMF Topic Modelling Results\n")
